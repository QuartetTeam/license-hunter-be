...
class CalendarControllerTest { // TODO 박현제: 인증/인가 완성 후 추가 테스트 작성 예정
    ...
    @Test
    @DisplayName("현재 사용자의 캘린더 목록을 조회할 수 있다")
    void getCurrentMemberCalendars_ShouldReturnCalendars() throws Exception {
        // given
        ...
        when(calendarService.getCalendarsByMemberId(anyLong())).thenReturn(responses);
        ...
        verify(calendarService, times(1)).getCalendarsByMemberId(1L);
    }

    @Test
    @DisplayName("자격증 캘린더를 구독할 수 있다")
    void subscribeCalendar_ShouldReturnCreatedStatus() throws Exception {
        // given
        ...
        // when
        final String result = mockMvc.perform(post("/api/v1/calendar/certifications/{certificationId}", certificationId)
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("자격증 캘린더 구독을 취소할 수 있다")
    void unsubscribeCalendar_ShouldReturnNoContentStatus() throws Exception {
        // given
        final long certificationId = 1L;
        ...
        // when
        final String result = mockMvc.perform(delete("/api/v1/calendar/certifications/{certificationId}", certificationId)
                        .contentType(MediaType.APPLICATION_JSON))
